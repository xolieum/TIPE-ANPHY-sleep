...
class MemmapSleepDataset(Dataset):
    ...
    def __getitem__(self, idx):
            ...
            # Suppression du unsqueeze(0) pour correspondre au Conv1d (Canaux, Temps)
            return torch.from_numpy(x).float(), torch.tensor(y).long()
    
# === Modèle CNN ===
class SleepStageCNN64(nn.Module):
    # Ici, l'argument s'appelle "input_channels"
    def __init__(self, input_channels=64, input_length=3000, num_classes=5):
        super().__init__()
        # On l'utilise pour définir la première couche
        self.conv1 = nn.Conv1d(input_channels, 32, kernel_size=3, padding=1)
        
        # Première couche : traite les 64 canaux simultanément
        self.conv1 = nn.Conv1d(input_channels, 32, kernel_size=3, padding=1)
        ...
        # Calcul automatique de la dimension de sortie pour les couches linéaires
        with torch.no_grad():
            # dummy shape: (1, 64, input_length)
...
def evaluate(model, loader):
    ...
    with torch.no_grad():
        for x_batch, y_batch in loader:
            ...
            # Comparaison sur le même device
